...
################## now going to plot the thetaS plots for the selected highlighted zone
 ...
 ## can also do this using a different technique  - let's try!
 drake.coords <- drake # saving this 
 
 # alrady have df with  reference coordinates #drake
 drake$cell <- 1:nrow(drake) # this is the polygon identifier

 #make into spatial object (probably better way to do this)
 coordinates(drake) <- ~x+y 
 plot(drake)
 
 
 drake.r <- raster(extent(drake), ncol = 1, nrow = 1) # 1x1 as we just want a rectangle
 drake.grid<-rasterize(drake, drake.r, drake$cell, FUN = max)
 drake.grid<-rasterToPolygons(drake.grid) #create polygons
 
 # plot this to check
 
 plot(drake.grid) # it's a signle box, so this is ok
 
 # now extract overlapping points
 points <- SpatialPoints(data.frame(x = r.overlap$ctdLON, y = r.overlap$ctdLAT))
 
 drake.points <- over(SpatialPolygons(drake.grid@polygons), SpatialPoints(points))
 
 plot(drake.points)
 
 # to plot in ggplot, we need to convery it back to a df
 drake.grid@data$id <- rownames(drake.grid@data)
  # now turn into a data.frame
 
 drake.gfor <- fortify(drake.grid, region = "id")
 
 # merge fortified data with data from spatial object
 drake.fortified <- merge(drake.gfor, drake.grid@data, by = "id")
 
 
 
 drake$pointsinpolygon <- over(SpatialPolygons(drake.grid@polygons), SpatialPoints(points))
 ...
 plot3 <- 
    ...
    geom_point(data = sites, aes(x = long, y = lat), size = 2, shape = 18) +
    coord_sf(xlim = c(-20, -79), ylim = c(-45, -70), expand = FALSE) + 
    geom_polygon(data = drake) 
 #, aes(x, y, group = Location, fill = Location), alpha = 0.5)    +
    #geom_label(data = polygon.labs, aes(LON, LAT, label = NAME), size = 5, fontface = "bold") +
    labs(x = "Longitude", y = "Latitude", colour = "Location") 
