...
for PlusDay in `seq $StartSeq $EndSeq`; do
  echo $PlusDay
  AddDay=`date -d "$PlusDay + 1 days"`
  echo $AddDay
  MyStartTime=`date -d "$YEAR-$MONTH-$DAY +$PlusDay days" +%Y-%m-%dT%H:%M:%SZ`
  MyEndTime=`date -d "$YEAR-$MONTH-$DAY +$AddDay days" +%Y-%m-%dT%H:%M:%SZ`
  ...
  URL="$NCSS/$MODEL/$EXPT?$VARS&$NORTH&$SOUTH&$EAST&$WEST&disableProjSubset=on&horizStride-1&time_start=1994-01-01T12%3A00%3A00Z&time_end=1994-01-02&T09%3A00%3A00Z&accept=netcdf4"
