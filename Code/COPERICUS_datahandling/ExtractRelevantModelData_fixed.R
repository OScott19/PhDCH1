...
all.model.data$ctdDEPTH <- NA
all.model.data$ctdSAL <- NA
all.model.data$ctdTEMP <- NA
...
for (i in 1:length(model.list)) {
  ...
  one.month.data$ctdDEPTH <- NA
  one.month.data$ctdSAL <- NA
  one.month.data$ctdTEMP <- NA
  ...
  for (x in days) {
    ...
    for (j in 1:length(day.locations.df$LAT)) {
      ...
      ### and now we are going to match it, because why the hell not
      ...
      ctd.matching <- ctd.from.day[ctd.from.day$LAT == day.lat & ctd.from.day$LONG == day.lon,]
      depths <- ctd.matching$DEPTH
      ...
      for (k in 1:length(temp.model$modDEPTH)) {
        ...
        single.model.depth <- temp.model$modDEPTH[k]
        # findind the ctd depth that is the closest to the depth of the model
        # ie matching a single model depth to the table of ctd depths 
        closest.depth <- dt.ctdDepth[J(single.model.depth), roll = "nearest"] # 
        # we then find where the matching lat is (for indexing purposes)
        closest.ref.depth <- match(x = closest.depth$val, 
                                   table = ctd.from.day$DEPTH[ctd.from.day$LAT == day.lat & ctd.from.day$LONG == day.lon])
        
        ## now we add in the comparison data
        
        temp.model$ctdDEPTH[k] <- closest.depth$val
        temp.model$ctdSAL[k] <- ctd.matching$SAL_A[closest.ref.depth]
        temp.model$ctdTEMP[k] <- ctd.matching$TEMP_P[closest.ref.depth]
  ...
  nc_close(model)
  print("saved to all.model, netcd file closed")
}

name.to.save.all <- "../Data/CMEMS_AllFixed_matched.Rdata"
...
test2 <- ggplot(data = world) +
  geom_sf() +
  geom_point(data = ctd.extracted, aes(x = LONG, y = LAT, col = "Original CTD locations"), size = 10, 
             shape = 18) +
  geom_point(data = all.model.data, aes(x = modlon.m1, y = modLAT.m1, colour = "Matched - method 1"), size = 8, 
             shape = 20) +
  geom_point(data = all.model.data, aes(x = modlon.m2, y = modLAT.m2, colour = "Matched - method 2"), size = 5, 
              shape = 18) +
  geom_point(data = all.model.data, aes(x = ctdLON, y = ctdLAT, col = "Stored CTD locations"), size = 2, 
             shape = 18) +
...
ggsave(filename = "../Figures/Map_CMEMSallmatchedCTDlocations.png")
