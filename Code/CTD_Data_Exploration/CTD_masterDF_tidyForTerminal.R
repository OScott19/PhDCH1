...
master.df <- data.frame(I = NA, DATE = NA, JULIANINFO = NA, 
                        ...
                        TEMP_P = NA, TEMP_A = NA, 
                        TEMP_UNIT = NA, TEMP_QUAL = NA,  
                        SAL_P = NA, SAL_A = NA, 
                        SAL_UNIT = NA, SAL_QUAL = NA, 
...
for (i in 1:length(r.df$file)) {
  # to ensure we're not carrying over data, we re-set the holding data frame to be empty each time
  ...
  ## different types of salinity need to be captured THERE ARE SEVEN
  
  # THESE are all of the different practical sailinity variable names
  
  if (length(w.data$var$PSALPR01) > 0) {
    holding.df$SAL_P <- ncvar_get(w.data, "PSALPR01")
    holding.df$SAL_UNIT <- w.data$var$PSALPR01$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "PSALPR01_SEADATANET_QC")
    }
  
  if (length(w.data$var$PSALST01) > 0) {
    holding.df$SAL_P <- ncvar_get(w.data, "PSALST01")
    holding.df$SAL_UNIT <- w.data$var$PSALST01$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "PSALST01_SEADATANET_QC")
    }
  
  if (length(w.data$var$PSALST02) > 0) {
    holding.df$SAL_P <- ncvar_get(w.data, "PSALST02")
    holding.df$SAL_UNIT <- w.data$var$PSALST02$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "PSALST02_SEADATANET_QC")
    }
  
  if (length(w.data$var$PSALCC01) > 0) {
    holding.df$SAL_P <- ncvar_get(w.data, "PSALCC01")
    holding.df$SAL_UNIT <- w.data$var$PSALCC01$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "PSALCC01_SEADATANET_QC")
    }
  
  if (length(w.data$var$PSALCU01) > 0) {
    holding.df$SAL_P <- ncvar_get(w.data, "PSALCU01")
    holding.df$SAL_UNIT <- w.data$var$PSALCU01$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "PSALCU01_SEADATANET_QC")
    }
  
  if (length(w.data$var$PSALCU02) > 0) {
    holding.df$SAL_P <- ncvar_get(w.data, "PSALCU02")
    holding.df$SAL_UNIT <- w.data$var$PSALCU02$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "PSALCU02_SEADATANET_QC")
    }
  
  ## and this is the only in-situ salinitiy 
  
  if (length(w.data$var$SSALPR01) > 0) {
    
    holding.df$SAL_A <- ncvar_get(w.data, "SSALPR01")
    holding.df$SAL_UNIT <- w.data$var$SSALPR01$units 
    holding.df$SAL_QUAL <- ncvar_get(w.data, "SSALPR01_SEADATANET_QC")
    }
  ...
  if (length(w.data$var$TEMPST01) > 0) {
    ...
    holding.df$TEMP_QUAL <- ncvar_get(w.data, "TEMPST01_SEADATANET_QC")
    }
  ...
  if (length(w.data$var$TEMPCC01) > 0) {
    ...
    holding.df$TEMP_QUAL <- ncvar_get(w.data, "TEMPCC01_SEADATANET_QC")
    }
  
  if (length(w.data$var$TEMPCU01) > 0) {
    holding.df$TEMP_A <- ncvar_get(w.data, "TEMPCU01")
    holding.df$TEMP_UNIT <- w.data$var$TEMPCU01$units 
    holding.df$TEMP_QUAL <- ncvar_get(w.data, "TEMPCU01_SEADATANET_QC")
    }
  ...
  if (length(w.data$var$TEMPPR01) > 0) {
    ...
    holding.df$TEMP_QUAL <- ncvar_get(w.data, "TEMPPR01_SEADATANET_QC")
    }
  ...
  if (length(w.data$var$POTMCV01) > 0) {
    ...
    holding.df$TEMP_QUAL <- ncvar_get(w.data, "POTMCV01_SEADATANET_QC")
    }
  ...
  if (length(w.data$var$POTMCV02) > 0) {
    ...
    holding.df$TEMP_QUAL <- ncvar_get(w.data, "POTMCV02_SEADATANET_QC")
    }
...
save(master.df, file = "../Data/CTD_master_211108_withQflags.Rdata")


print("Data saved, script finished, time to run the post-terminal script")
